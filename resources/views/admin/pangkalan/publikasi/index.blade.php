...
@section('content')
    ...
    <div class="row">
        <div class="col-12 col-md-6 order-md-1 order-last">
            <h3>Publikasi</h3>
            <p class="text-subtitle text-muted">Menampilkan daftar publikasi dosen prodi.</p>
        </div>
    ...
    <div class="card">
        <div class="row">
            ...
            <div class="col-sm mt-3" style="text-align: right">
                <a href="/prodi/data/publikasi/create"><button type="button" class="btn btn-primary"
                        style="margin-right: 15px"><i class="bi bi-plus-square"
                            style="margin-right:7px;position: relative;top: -1px;"></i> Tambah Publikasi</button></a>
            </div>
        </div>
            ...
            <div class="table-responsive text-wrap">
                <table class="table table-striped" id="example">
                    ...
                    <tbody class="table-border-bottom-0">
                        ...
                        @foreach ($publikasi as $pn)
                            <tr>
                                <td style="padding-left: 20px">{{ $loop->iteration }}</td>
                                <td>{{ ucwords($pn['judul']) }}</td>
                                <td>{{ ucwords($pn['penulis_dosen']) }}</td>
                                <td>
                                    @if ($pn['anggota'])
                                        @foreach (json_decode($pn['anggota']) as $item)
                                        {{ $loop->iteration }}. {{ $item->dosen }} <br>
                                        @endforeach
                                    @endif

                                </td>
                                <td>{{ $pn['tahun'] }}</td>
                                <td>{{ ucwords($pn['jenis']) }}</td>
                                <td>{{ ucwords($pn['sub_jenis']) }}</td>
                                <td>{{ $pn['sitasi'] }}</td>
                                <td width="100">
                                    <div class="d-flex text-nowrap">
                                        <a href="/prodi/data/publikasi/{{ $pn->ids }}/edit"><button type="button"
                                                class="btn btn-sm btn-icon btn-primary" style="margin-left: 5px"><i
                                                    class="bi bi-pencil-fill"></i></button></a>
                                        <form action="/prodi/data/publikasi" method="post" style="margin-left: 5px">
                                            @csrf
                                            @method('delete')
                                            <input type="hidden" name="ids" id="ids" value="{{ $pn->ids }}"
                                                hidden>
                                            <button type="submit" class="btn btn-sm btn-icon btn-danger"
                                                onclick="return confirm('Yakin menghapus publikasi ini ?')"><i
