...
class KerjasamaController extends Controller
{
    ...
    {
        return view('portaldata.kerjasama');
    }
    ...
    public function get_data(Request $request)
    {
        ...
        $query->with(['prodi' => function ($query) {
            $query->with('kerjasama');
        }]);
        $data = $query->get();

        //Membuat variabel di collection $data
        $all_data['total_kerjasama'] = 0;
        ...
        foreach ($data as $d) {
            ...
            foreach ($d['prodi'] as $p) {
                ...
                //Mengolah data $data
                foreach ($p['kerjasama'] as $m) {

                    $all_data['total_kerjasama'] += 1;
                    $d['total_kerjasama'] += 1;
                    $p['total_kerjasama'] += 1;
                    ...
                    if ($m['bidang'] == 'pendidikan') {
                        $sumber = 0;
                    } elseif ($m['bidang'] == 'penelitian') {
                        $sumber = 1;
                    } elseif ($m['bidang'] == 'pengabdian kepada masyarakat') {
                        $sumber = 2;
                    } elseif ($m['bidang'] == 'pengembangan kelembagaan') {
                        $sumber = 3;
                    }
